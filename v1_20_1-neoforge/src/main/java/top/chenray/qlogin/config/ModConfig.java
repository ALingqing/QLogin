...
public class ModConfig {
    ...
    public static ModConfig load(Path configDir) {
        ...
        if (Files.exists(configPath)) {
            try {
                ...
                logger.error("加载配置失败", e); instance = new ModConfig();
            }
        } else {
            instance = new ModConfig(); save();
        }
        return instance;
    }
    ...
    public static void save() {
        ...
        try { Files.createDirectories(configPath.getParent()); Files.writeString(configPath, GSON.toJson(instance)); }
        catch (IOException e) { LoginMod.LOGGER.error("保存配置失败", e); }
    }
}
