    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("qlogin")
            ...
            .then(Commands.literal("reload")
                .executes(context -> {
                    ModConfig.load(context.getSource().getServer().getServerDirectory().toPath()
                        .resolve("config").resolve("loginmod"));
                    ...
                    .then(Commands.argument("newPassword", StringArgumentType.word())
                        .executes(context -> {
                            ...
                            if (!DatabaseManager.getInstance().isPlayerRegistered(target.getUUID())) {
                                context.getSource().sendFailure(Component.literal("§c该玩家尚未注册"));
                                return 0;
                            }
