...
/**
 * /changepassword <旧密码> <新密码> - 修改密码命令 (Mojang 映射版)
 */
public class ChangePasswordCommand {
    ...
    private static int executeChangePassword(ServerPlayer player, String oldPassword, String newPassword) {
        ...
        if (newPassword.length() < config.getPasswordMinLength() || newPassword.length() > config.getPasswordMaxLength()) {
            player.sendSystemMessage(Component.literal("§c新密码长度必须在 " + config.getPasswordMinLength() + "-" + config.getPasswordMaxLength() + " 个字符之间"));
            return 0;
        }
        ...
        if (db.changePassword(player.getUUID(), newPassword)) {
            ...
            top.chenray.qlogin.LoginMod.LOGGER.info("玩家 {} 已修改密码", player.getScoreboardName());
            return 1;
        } else {
